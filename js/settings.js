...
FamilyOffice.Settings = (function () {
    ...
    function initEvents() {
        ...
        var syncToggle = document.getElementById('cloud-sync-toggle');
        if (syncToggle) {
        ...
        if (pushBtn) {
            pushBtn.addEventListener('click', function () {
                var Supabase = FamilyOffice.Supabase;
                if (!Supabase || !Supabase.pushToCloud) {
                    showNotification('Supabase not available', 'error');
                    return;
                }
                pushBtn.disabled = true;
                pushBtn.innerHTML = '<span class="btn-icon">⏳</span> Pushing...';
                showSyncStatus('Pushing data to cloud...', 'info');

                Supabase.pushToCloud()
                    .then(function (result) {
                        showSyncStatus('✅ Pushed ' + result.companies + ' companies, ' + result.founders + ' founders, ' + result.funds + ' funds to cloud!', 'success');
                        showNotification('Data pushed to cloud!', 'success');
                        render();
                    })
                    .catch(function (err) {
                        showSyncStatus('❌ Push failed: ' + err, 'error');
                        showNotification('Push failed: ' + err, 'error');
                        pushBtn.disabled = false;
                        pushBtn.innerHTML = '<span class="btn-icon">⬆️</span> Push to Cloud';
                    });
            });
        }
        ...
        if (pullBtn) {
            pullBtn.addEventListener('click', function () {
                ...
                pullBtn.innerHTML = '<span class="btn-icon">⏳</span> Pulling...';
                showSyncStatus('Pulling data from cloud...', 'info');

                Supabase.pullFromCloud()
                    .then(function (result) {
                        showSyncStatus('✅ Pulled ' + result.companies + ' companies, ' + result.founders + ' founders, ' + result.funds + ' funds from cloud!', 'success');
                        showNotification('Data pulled from cloud!', 'success');
                        render();
                    })
                    .catch(function (err) {
                        showSyncStatus('❌ Pull failed: ' + err, 'error');
                        showNotification('Pull failed: ' + err, 'error');
                        pullBtn.disabled = false;
                        pullBtn.innerHTML = '<span class="btn-icon">⬇️</span> Pull from Cloud';
                    });
            });
        }
